...
def runAlignedDataCase(K=5):
    ...
    for hm in('Cu','Mg'):
        ...
        for env in('dTRI','nlights'):
            ...
            yMatrix_al_new = y_star_flat.reshape(y.shape)
            results_sampling = run_GCCM_sampling(xMatrix_al, yMatrix_al_new, lib_sizes, E=4, cores=6)
            ...
            plt.savefig('rTrend/differentK/'+enviNames[env]+'_'+hm+'K='+str(K)+'.png', bbox_inches='tight')
            results.to_csv('rTrend/differentK/'+enviNames[env]+'_'+hm+'K='+str(K)+'.csv', index=False)

if __name__ == "__main__":
    runAlignedDataCase(K=5)
    runAlignedDataCase(K=50)