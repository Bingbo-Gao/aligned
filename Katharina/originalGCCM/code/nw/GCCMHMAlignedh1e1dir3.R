...
x_xmap_y <- GCCM(xMatrix, yMatrixM, lib_sizes, lib, pred, E,tau = 1, b = E+2, winStepRatio=0,cores=32,dir=3,validRatio=0) #
y_xmap_x <- GCCM(yMatrixM, xMatrix, lib_sizes, lib, pred, E,tau = 1, b = E+2, winStepRatio=0,cores=32,dir=3,validRatio=0) #
...
if (length(x_xmap_y_means) == length(y_xmap_x_means)){
  write.csv(results_Cu, file=paste("resultdir3/",yName,xName,"_Eis_",E,".csv",sep = ""))
}else{
  print("行号不一致")
  save.image(file =paste("resultdir3/",yName,xName,"_Eis_",E,".RData",sep = ""))
...
jpeg(filename = paste("resultdir3/",xName,"_",yName,"_Eis",E,"_.jpg",sep = ""),width = 600, height = 400)
