...
load("MgCualigned.RData")


HMs<-c("Cu_aligned","Mg_aligned")

Envs<-c("dTRI_aligned","nlights_aligned")
...
x_xmap_y <- GCCM(xMatrix, yMatrixM, lib_sizes, lib, pred, E,tau = 1, b = E+2, winStepRatio=0,cores=32,dir=0,validRatio=0) #
y_xmap_x <- GCCM(yMatrixM, xMatrix, lib_sizes, lib, pred, E,tau = 1, b = E+2, winStepRatio=0,cores=32,dir=0,validRatio=0) #
...
results_Cu<-data.frame(lib_sizes,x_xmap_y_means,y_xmap_x_means,x_xmap_y_Sig,y_xmap_x_Sig,x_xmap_y_interval,y_xmap_x_interval)  #Save the cross-mapping prediciton results

if (length(x_xmap_y_means) == length(y_xmap_x_means)){
  write.csv(results_Cu, file=paste("resultdir0/",yName,xName,"_Eis_",E,".csv",sep = ""))
}else{
  print("行号不一致")
  save.image(file =paste("resultdir0/",yName,xName,"_Eis_",E,".RData",sep = ""))
...
jpeg(filename = paste("resultdir0/",xName,"_",yName,"_Eis",E,"_.jpg",sep = ""),width = 600, height = 400)
